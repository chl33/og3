...
#ifndef NATIVE
    if (m_web_server) {
      m_web_server->on(cfg_url(),
                       [this](NetRequest* request) { return this->handleConfigRequest(request); });
    }
#endif
  });
}
...
NetHandlerStatus ConfigModule::handleConfigRequest(NetRequest* request) {
#ifndef NATIVE
